...
export const clickhouseHost = secret(
  "CH_HOST",
  exampleSecretValue("CH_HOST", process.env.CH_HOST),
  {
    description:
      "ClickHouse Cloud HTTPS endpoint for the otel database (e.g. https://woucf8396t.us-east-1.aws.clickhouse.cloud:8443).",
    errorMessage: "Set process.env.CH_HOST (Infisical /apps/otel-relay).",
  },
);

export const clickhouseUser = secret(
  "CH_USER",
  exampleSecretValue("CH_USER", process.env.CH_USER),
  {
    description: "ClickHouse user with read access to the otel database.",
    errorMessage: "Set process.env.CH_USER (Infisical /apps/otel-relay).",
  },
);

export const clickhouseKey = secret(
  "CH_KEY",
  exampleSecretValue("CH_KEY", process.env.CH_KEY),
  {
    description: "ClickHouse password / API key for the read-only otel user.",
    errorMessage: "Set process.env.CH_KEY (Infisical /apps/otel-relay).",
  },
);

export const githubToken = secret(
  "GITHUB_TOKEN",
  exampleSecretValue("GITHUB_TOKEN", process.env.GITHUB_TOKEN),
  {
    description:
      "GitHub token with read access to commits for regression correlation.",
    errorMessage: "Set process.env.GITHUB_TOKEN for the SRE monitor.",
  },
);
...
// Server spans (root spans) only, so we measure full request latency
// per the schema notes in .agents/skills/observability/SKILL.md.
const LATENCY_SQL = `
  SELECT
    count()                              AS requests,
...
// Recurring sweeps. These declarations live alongside the workflow itself so
// the schedule travels with the deployment — the executing backend (any
// platform that drives /schedules/tick) picks them up automatically.
  ...
  payload: {
    ...
    slackChannel:
      typeof process !== "undefined"
        ? (process.env.SRE_SLACK_CHANNEL ?? "#sre")
        : "#sre",
